 ...
 * 
 * This example shows an interactive explanation of Cyclic Coordinate Descent algorithm (CCD).
 *
 * Here it is generated a Chain Structure (either constrained or not) and a Target (interactive) that
 * the End Effector must reach. Then a CCDSolver is instantiated and related to the structure. 
 * 
 * Press 's' to reach the Target using CCD.
 * Press 'e' to see step by step how CCD works.
...
void setup() {
    ...
    //Add constraints (look at Util tab)
    generateConstraints(structure,ConstraintType.NONE, true);
...
void keyPressed(){
    if(key == 'e'){
        ...
        solver.solve();
        if(CCDAnimator == null) CCDAnimator = new IKAnimation.CCDAnimation(auxiliar, solver, radius, chainColor);
        ...
        if(CCDAnimator != null) CCDAnimator.setPeriod(max(Integer.valueOf("" + key) * 1000, 100));
    }
}
