class Boid extends Node {
  ...
  Boid(Node obj, Skinning skinning, Vector inPos, ArrayList<Boid> flock) {
    position = new Vector();
    position.set(inPos);
    setPosition(new Vector(position.x(), position.y(), position.z()));
    velocity = new Vector(EagleFlock.this.random(-1, 1), EagleFlock.this.random(-1, 1), EagleFlock.this.random(1, -1));
    ...
    task = new TimingTask() {
      ...
      public void execute() {
        ...
        acceleration.add(Vector.multiply(cohesion, 1));
        acceleration.add(Vector.multiply(separation, 4));
        move();
        checkBounds();
      }
    };
