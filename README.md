...
3. The default _hid_ is defined with a `null` String parameter (e.g., [scale(String, float)](https://visualcomputing.github.io/frames-javadocs/frames/core/Graph.html#scale-java.lang.String-float-) simply calls `scale(null, delta)`). The _Scene_ default mouse _hid_ presented in the [Frames](#user-content-frames) section is precisely implemented is this manner.
...
Override [Frame.interact(java.lang.Object... gesture)](https://visualcomputing.github.io/frames-javadocs/frames/core/Frame.html#interact-java.lang.Object...-) to parse the gesture into a custom (application) control. The frame interaction pattern would then look like: ```interact(Frame frame, Object... gesture) = frame.interact(gesture)```. You may also implement an _hid_ interaction pattern: either ```interact(String hid, Object... gesture) = interact(scene.defaultFrame(hid), gesture)```, for general _hids_; or ```interact(Object... gesture) = interact(scene.defaultFrame(), gesture)```, for the default _hid_. See the [ApplicationControl](https://github.com/VisualComputing/frames/tree/master/examples/demos/ApplicationControl) for an example of the latter pattern.
