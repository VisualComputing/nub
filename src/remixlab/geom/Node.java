...
public class Node extends Frame implements Grabber {
  ...
  */
  ...
  public void setFOVToFitScene() {
    if (Vector.scalarProjection(Vector.subtract(position(), graph().center()), zAxis()) > (float) Math.sqrt(2.0f) * graph().radius())
      setFieldOfView(2.0f * (float) Math.asin(graph().radius() / Vector.scalarProjection(Vector.subtract(position(), graph().center()), zAxis())));
    else
  ...
  public void setHorizontalFieldOfView(float hfov) {
    setFieldOfView(2.0f * (float) Math.atan((float) Math.tan(hfov / 2.0f) / graph().aspectRatio()));
  }
}
