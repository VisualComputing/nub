...
public class ForwardTRIK extends Solver {
    protected Context _context;
    protected Heuristic _forwardHeuristic;
    ...
    public ForwardTRIK(List<? extends Node> chain) {
        this(chain, null);
    }
    ...
    protected boolean _iterateStepByStep(){
        ...
        if(_stepCounter == 0) {
            ...
            _forwardHeuristic.prepare();
            _stepCounter++;
        } else if(_stepCounter - 1 < _context.chain().size()){
            //for(int i = 1; i < _chain.size(); i++) {
            int i = _stepCounter - 1;
            _forwardHeuristic.applyActions(i - 1);
            _context.usableChainInformation().get(i).updateCacheUsingReference();
        } else{
    ...
    protected boolean _iterate() {
        ...
        for(int i = 0; i < _context.chain().size() - 1; i++) {
            _forwardHeuristic.applyActions(i);
