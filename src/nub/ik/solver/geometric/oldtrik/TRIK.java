...
public class TRIK extends Solver {
  ...
  protected boolean _iterate() {
    ...
    if (_positionError(_original_info.get(last).positionCache(), _worldTarget.position()) < _maxError && _orientationError(_original_info.get(last).orientationCache(), _worldTarget.orientation(), true) < 1)
      return true;
    return false;
  }
...
*/
    ...
    if (explore) {
      if (j_i.node().constraint() == null || !(j_i.node().constraint() instanceof Hinge)) {
        ...
        for (int c = 0; c < 0; c++) {
          Quaternion q = null;
          if (c % 2 == 0)
            q = new Quaternion(axis, (float) Math.toRadians(Math.random() * Math.min(Math.toDegrees(max), 45)));
  ...
  protected List<Quaternion> _lookAhead(List<NodeInformation> chain, List<NodeInformation> auxiliary_chain, Node target, boolean direction, boolean enableWeight, boolean explore, boolean enableTwist, boolean smooth, float smoothAngle, LookAheadMode mode, int from, int times, int depth, List<Quaternion> sequence, List<Pair<List<Quaternion>, Float>> actions, float movement) {
    ...
    if (depth == times) {
      ...
      float e2 = 0;
      if (_debug)
        System.out.println("e1 : " + Math.toDegrees(e1) + " e2: " + Math.toDegrees(e2) + "e_tot" + Math.toDegrees(e1 + e2 * 0.1f) + "mov " + Math.toDegrees(movement));
    ...
    if (depth == 0) {
      ...
      for (Pair<List<Quaternion>, Float> action : actions) {
        if (_debug)
          System.out.println("Action a " + i++ + "   " + action.getKey().get(0).axis() + action.getKey().get(0).angle() + " err " + Math.toDegrees(action.getValue()));
