...
public class TreeSolver extends FABRIKSolver {
  ...
  protected boolean _addTargetDirection(TreeNode treeNode, Node endEffector, Vector direction) {
    if (treeNode == null) return false;
    if (treeNode._solver().endEffector() == endEffector) {
  ...
  public boolean addTargetDirection(Node endEffector, Vector direction) {
    return _addTargetDirection(root, endEffector, direction);
  }

  public TreeSolver(Node frame) {
    super();
    TreeNode dummy = new TreeNode(); //Dummy TreeNode to Keep Reference
    _setup(dummy, frame, new ArrayList<Node>());
