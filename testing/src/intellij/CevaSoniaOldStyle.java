...
/**
 * Port of Jacques Maire proscene sketch (old style).
 * https://t.ly/bsgVv
 */
public class CevaSoniaOldStyle extends PApplet {
  Scene scene;
  Ceva ceva;
  PImage abeille;
  ...
  public void mouseMoved() {
    scene.updateTag(scene.mouseX(), scene.mouseY(), ceva.reperes);
  }
  ...
  class Ceva {
    ...
    Ceva() {
      ...
      for (int i=0;i<nb;i++) {
        reperes[i]=new Node();
        reperes[i].enableHint(Node.BULLSEYE, Node.BullsEyeShape.CIRCLE);
        reperes[i].setBullsEyeSize(0.2f);
    ...
    void cevaDraw() {
      for ( int i=0;i<nb;i++) {
        pushMatrix();
        applyMatrix(Scene.toPMatrix(reperes[i].worldMatrix()));
  ...
  public static void main(String[] args) {
    PApplet.main(new String[]{"intellij.CevaSoniaOldStyle"});
  }
}
