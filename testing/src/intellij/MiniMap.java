...
public class MiniMap extends PApplet {
  ...
  public void setup() {
    ...
    if (renderer == P3D)
      minimap.setAperture(Graph.Type.PERSPECTIVE);
    minimap.fitBallInterpolation();
    sceneEye = new Frame();
  }

  PShape box() {
    PShape box = renderer == P3D ? createShape(BOX, 60) : createShape(RECT, 0, 0, 80, 100);
  ...
  public void draw() {
    ...
    if (displayMinimap) {
      ...
      minimap.frontBuffer().fill(0, 255, 0);
      minimap.frontBuffer().stroke(0, 0, 255);
      minimap.frontBuffer().strokeWeight(2);

      if (interactiveEye) {
        minimap.frontBuffer().pushMatrix();
        minimap.applyTransformation(sceneEye);
        minimap.drawEye(minimap.frontBuffer(), scene);
        minimap.frontBuffer().popMatrix();
      } else
